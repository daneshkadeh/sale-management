...
/**
 * TODO phamcongbang the purpose of this class ??? The class name is so common. We should separate the function relate
...
public class ControlConfigUtils {
    ...
    private static Font s_fontItalic;
     ...
     * @param text
     * @param searchChar
     * @return string after deleting single character.
     */
    public static String deleteSingleChar(String text, char searchChar) {
        if (text == null) {
            return text;
        }

        int pos = findSeparator(text, searchChar, -1);
        while (pos >= 0) {
            text = text.substring(0, pos) + text.substring(pos + 1);
            pos = findSeparator(text, searchChar, pos + 1);
        }
        return text;
